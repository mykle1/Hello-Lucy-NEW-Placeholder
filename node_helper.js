...
var loadingRules=fs.readFileSync('modules/Hello-Lucy/checkCommands.json', 'utf8');
...
module.exports = NodeHelper.create({
    ...
    checkFiles() {
        ...
        fs.stat('modules/Hello-Lucy/words.json', (error, stats) => {
            if (!error && stats.isFile()) {
                fs.readFile('modules/Hello-Lucy/words.json', 'utf8', (err, data) => {
                    if (!err) {
    ...
    generateDicLM() {
        ...
        fs.writeFile('modules/Hello-Lucy/words.json', JSON.stringify({ words: this.words }), (err) => {
            if (err) {
        ...
        lmtool(this.words, (err, filename) => {
            if (err) {
                ...
                fs.renameSync(`${filename}.dic`, 'modules/Hello-Lucy/Hello-Lucy.dic');
                fs.renameSync(`${filename}.lm`, 'modules/Hello-Lucy/Hello-Lucy.lm');
    ...
    logDebug(data) {
        fs.appendFile('modules/Hello-Lucy/debug.log', data, (err) => {
            if (err) {
    ...
    logError(error) {
        if (error) {
            fs.appendFile('modules/Hello-Lucy/error.log', `${error}\n`, (err) => {
                if (err) {
                    console.log(`${this.name}: Couldn't save error to log file!`);
                }
